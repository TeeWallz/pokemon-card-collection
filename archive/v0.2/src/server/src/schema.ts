...
export const resolvers = {
  Query: {
    ...
    collectionsPagination: async (parent, args, ctx: Context) => {
      // utils.getUserId(ctx)
      const take = 10
      const skip = (args.page - 1) * take
      ...
      const count = await ctx.prisma.collection.count({ where })
      return { collections, count, take }
    },
  },
  Mutation: {
    ...
    createCollection: async (parent, args, ctx: Context) => {
      // const userTest = await ctx.prisma.user.findUnique({
      //   where: {
      //     email: args.email,
      //   },
